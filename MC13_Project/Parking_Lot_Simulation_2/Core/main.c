...
volatile uint32_t parking_space_remain = 5;
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
  // PIN PLAN
  // 1. Use Pins PC0 as Sensor for Exit
  // 2. Use Pins PC1 as Sensor for Entrance
  // 3. Use Pins PB2 as Output Pin to Drive Exit LED
  // 4. Use Pins PB3 as Output Pin to Drive Entrance LED
  // 5. Use Pins PB6 as Output Pin to Drive VCC of 7-segment display
  // 6. Use Pins PB7 as Output Pin to Drive DIO of 7-segment display
  ...
  GPIOB -> MODER &= 0xFFFFFF0F;			// Mask the irrelevant pins
  GPIOB -> MODER |= 0x00000050;	// Configure PB2 and PB3 as output pins
  GPIOB -> PUPDR |= 0x000000A0; 	// Pull down PB2 and PB3 (Optional)


  RCC -> APB2ENR = 0x4000;				// Enable for Interrupts
  SYSCFG -> EXTICR[0] = 0x0022;			// PCX for both PC0 and PC1
  EXTI -> IMR = 0x0003;				// Mask
  EXTI -> RTSR = 0x0003;				// Rising Edge Trigger

  NVIC_EnableIRQ(EXTI0_IRQn);			// Enable Interrupt Line 0
  NVIC_EnableIRQ(EXTI1_IRQn);			// Enable Interrupt Line 1
  ...
  GPIOB -> ODR |= 0x00C0;			// Set PB6 and PB7 as HIGH */
	...
	tm1637_delay_long(2600000);		//HAL_Delay(2000);

	__enable_irq();  // Enables global interrupts (nested ISRs now allowed)
	while(1){
...
{
	if(parking_space_remain < 200){
		parking_space_remain++;
	}
	GPIOB->ODR |= (1 << 2);
	tm1637_delay_long(1300000);
	GPIOB->ODR &= (0 << 2);
	display_info();
	EXTI -> PR = 0x0001;
}
...
void display_info(void){
	if(parking_space_remain == 0){
				...
				tm1637_delay_long(650000);//HAL_Delay(500);
